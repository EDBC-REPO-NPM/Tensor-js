const { Tensor } = require('./TensorJS/main.js');
...
var mlp = new Math.MLP();

mlp.addLayer(4, sigmoid, input_layer);
mlp.addLayer(5, sigmoid, hidden_layer);
mlp.addLayer(3, softmax, output_layer);
mlp.compile();

for( var i=500; i--; ){ for( var j in dataset ){ 
	console.log('done',i,j);
	mlp.fit( dataset[j][0],dataset[j][1] );
}}

for( var j in test ){ 
	mlp.predict( test[j] );
	mlp.getOutput().showTensor(4);
}